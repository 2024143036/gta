...
def main():
    ...
    path = "data/auto_labeled_result.csv"
    try:
        df = pd.read_csv(path)
    ...
    # 8. 모델 저장
    print("\n=== 모델 저장 ===")
    model.save_pretrained('gta_mobilebert_auto')
    tokenizer.save_pretrained('gta_mobilebert_auto')
    print(" 모델 가중치 및 토크나이저 저장 완료 ('gta_mobilebert_auto' 폴더)")
    ...
    result_df.insert(1, "dataset", "auto_label")
    ...
    summary_df.to_csv(
        "result/mobilebert_auto_summary.csv",
        index=False,
        encoding="utf-8-sig"
    )

    print("저장 완료:")
    print(" - result/mobilebert_auto_history.csv")
    print(" - result/mobilebert_auto_summary.csv")
